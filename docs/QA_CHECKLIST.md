...
- WU-09-03 evidence binds Today to the owner-approved visual Source of Truth and covers 12 fresh states: populated light/dark, past/current/future, current divider, long title, alarm OFF, empty, Large/Accessibility XXXL, and small/standard/large iPhones.
- WU-09-04 evidence covers the two-tab `アラーム / 設定` navigation and 12 unified-timeline/detail/settings/calendar scenarios. The root-owned presentation clock advances only while active, refreshes immediately after lifecycle/time signals, coalesces repeated activation, and cannot fetch, reconcile, persist, or schedule on a minute tick.
- WU-09-05 evidence covers 12 lightweight onboarding, Calendar/Alarm rationale and denied, completed-app, Settings, and Event Detail states. Onboarding completion is independent of authorization, requests occur only from `.notDetermined`, and revocation stays in normal recovery UI.
- Alarm presentation uses the exact nonblank source event title without prefixes or suffixes; blank/whitespace-only titles use `予定`. Alarm date, stable identity, UUID isolation, scheduling lifecycle, and reconciliation semantics remain regression-tested.
- Today uses the hierarchy event title > event time > exact alarm time > supporting state. Visible per-row calendar labels are omitted for scanability but remain in conceptual accessibility labels.
- Today inserts the actual current time deterministically between chronologically ordered events. Its filled marker, `現在` label, and dashed rule are presentation-only and must not invoke scheduling or reconciliation.
...
- The unified `アラーム` display window includes the previous 14 days and next 14 days, remains chronological and read-only, initially positions once near current time, exposes `現在へ`, and does not force-scroll on presentation ticks. It must not broaden the reconciliation window or schedule past alarms.
...
- WU-19 reviewed Packaging Commit `e35868b0612d707c476fa51f2e1272bd9797850e`, observed State Snapshot `b210f0bbe6f751435fd307608081f272b81ad6ed`: read-only public baseline 1.0 (3) / highest upload 3 / no draft or in-review version; candidate 1.0.1 (4) NOT YET RELEASED; production functional delta 0; XCTest 182 passed / 0 failed / 0 skipped; all four builds, fresh signed Release archive, local App Store export, strict codesign, identity/entitlements/privacy/CTA audit PASS. Physical WU-18 Gate is previously PASS on reviewed implementation lineage, not a new physical test. Upload/submission NOT STARTED; ChatGPT Release Candidate Review PASS. Review Sync is docs-only; retain the exact reviewed archive/IPA and stop after fast-forward-only main closure.
