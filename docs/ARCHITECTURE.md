...
- The task identifier is `com.naoki-ko.agendacue.refresh`; `Info.plist` permits that identifier and enables only the `fetch` background mode. The requested earliest begin is six hours after scheduling, not a promised execution time.
