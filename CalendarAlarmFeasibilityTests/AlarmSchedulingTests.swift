...
final class AlarmSchedulingTests: XCTestCase {
    ...
    func testBlankTitleUsesDeterministicJapaneseFallback() async throws { let x = setup(); _ = try await x.coordinator.schedule(candidate(title: "  ")); XCTAssertEqual(x.system.lastTitle, "予定") }
    func testJapaneseAndSourceTitlesArePassedExactlyWithoutPrefixOrSuffix() async throws { let x = setup(); _ = try await x.coordinator.schedule(candidate(title: "歯医者 / Team Sync")); XCTAssertEqual(x.system.lastTitle, "歯医者 / Team Sync"); XCTAssertFalse(x.system.lastTitle!.contains("Calendar Alarm")); XCTAssertFalse(x.system.lastTitle!.contains("カレンダー")) }
    func testTitleMappingDoesNotAlterDateIdentityOrPersistedUUID() async throws { let x = setup(); let value = candidate(date: .init(timeIntervalSince1970: 4_000), title: "出勤"); guard case .scheduled(let id) = try await x.coordinator.schedule(value) else { return XCTFail() }; XCTAssertEqual(x.system.lastDate, value.alarmDate); XCTAssertEqual(x.store.value?.candidateIdentity, value.id); XCTAssertEqual(x.store.value?.alarmIdentifier, id) }
