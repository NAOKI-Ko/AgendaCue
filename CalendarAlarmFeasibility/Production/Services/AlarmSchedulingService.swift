...
actor AlarmSchedulingCoordinator {
    ...
    private func presentationTitle(_ title: String) -> String { title.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? ProductionCopy.fallbackEventTitle : title }
}
...
actor AlarmKitSystemScheduler: AlarmSystemScheduling {
    ...
    func schedule(id: UUID, at date: Date, title: String) async throws {
        ...
        let alert = AlarmPresentation.Alert(title: localized, stopButton: AlarmButton(text: LocalizedStringResource("alarm.stop"), textColor: .white, systemImageName: "stop.fill"))
